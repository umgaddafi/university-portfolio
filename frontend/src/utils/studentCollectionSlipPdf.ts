...
export type StudentCollectionSlipPayload = {
  name: string
  matric: string
...
type GenerateStudentCollectionSlipPdfParams = {
  payload: StudentCollectionSlipPayload
  autoPrint?: boolean
}
...
function drawSignatureBox({
  ...
  // pdf.text("Sign by hand before collection.", x + 7, y + height - 3.5)
}

function buildFileName(payload: StudentCollectionSlipPayload) {
  const normalizedMatric = payload.matric.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "")
  return `${payload.filePrefix ?? "student-collection-slip"}-${normalizedMatric || "record"}.pdf`
}

export async function generateStudentCollectionSlipPdf({
  payload,
  autoPrint = false,
}: GenerateStudentCollectionSlipPdfParams): Promise<{ blobUrl: string; fileName: string }> {
  ...
  const serviceLabel = payload.serviceLabel ?? "Student ID Services"
  const slipTitle = payload.slipTitle ?? "Student ID Card Collection Slip"
  const personLabel = payload.personLabel ?? "Student Name"
  const identifierLabel = payload.identifierLabel ?? "Matric Number"
  const passportLabel = payload.passportLabel ?? "Student Passport"
  const acknowledgementTitle = payload.acknowledgementTitle ?? "Student Acknowledgement"
  const pickupFallbackMessage =
    payload.pickupFallbackMessage ?? "Collect your ID card from the designated Student ID desk."
  const footerNote =
    payload.footerNote ?? "This slip should be signed and submitted during student ID card pickup."
  ...
  pdf.text(serviceLabel, PAGE_WIDTH_MM / 2, PAGE_MARGIN_TOP_MM + 9.2, { align: "center" })
  pdf.text(slipTitle, PAGE_WIDTH_MM / 2, PAGE_MARGIN_TOP_MM + 13.8, {
    align: "center",
  })
  ...
  pdf.setFont("helvetica", "normal")
  pdf.setFontSize(5.4)
  pdf.text(`Issued ${new Intl.DateTimeFormat("en-NG", { dateStyle: "medium" }).format(new Date())}`, PAGE_WIDTH_MM - PAGE_MARGIN_X_MM - 20, PAGE_MARGIN_TOP_MM + 8.1, {
    align: "center",
  })
  ...
  pdf.setFontSize(7)
  pdf.setTextColor(71, 85, 105)
  pdf.text(passportLabel, photoBoxX + photoBoxWidth / 2, detailsTopY + photoBoxHeight - 2.5, {
    align: "center",
  })
  ...
  const pickupPanelBottom = drawTextPanel({
    ...
    body: payload.pickupMessage || pickupFallbackMessage,
    x: PAGE_MARGIN_X_MM,
    y: pickupPanelY,
